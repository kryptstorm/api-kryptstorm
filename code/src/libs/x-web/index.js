...
export default function XWeb({ authentication = '', publicRoutes = [] }) {
	const seneca = this;
	...
	server.use((req, res, next) => {
		...
		if (authentication && (!seneca.has(authentication) || !_.isString(authentication))) {
	...
	return { name: 'XWeb', exportmap: { server: (routes, isDebug = false) => _server.bind(this, server, routes, isDebug)() } };
}
...
function _prepareOrder(orderBy = 'id', asc) {
	if (!_.isEmpty(orderBy) && _.isString(orderBy)) {
		return { [orderBy]: asc == 1 ? 'ASC' : 'DESC' };
	}
	return { id: 'DESC' };
}
function _preparePagination(limit, page) {
...
function _server(server, routes = [], isDebug = false) {
	const seneca = this;
